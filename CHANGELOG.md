...
- Updated `docs/rules/README.md` header to point at v0.3.0 instead of v0.1.1; fixed stale `suggest` action labels on OA006/OA007.
...
- Wholesale doc pass to remove em dashes for consistent ASCII-only punctuation.
...
- 11 new tests covering logger behavior and record emission.
...
```
Produces a 6-line NDJSON stream covering attempt to 3 applied to 1 failed to complete, including the full RFC 6902 ops for each applied patch and the failure reason for the one that didn't land.
...
- New tests covering: multi-op patch emission, existing-parent-override to replace path, container mirroring (npm vs pnpm.overrides), OA007 replace patch, and end-to-end multi-op fix application in the orchestrator.
...
- `src/fixer/apply.ts`: RFC 6902 applier supporting `remove`/`replace`/`move`/`add`. Auto-creates intermediate objects on `add`/`move` so OA003's pnpm.overrides to overrides move works on projects that never had a top-level overrides block.
- `src/fixer/write.ts`: indent detection (2-space / 4-space / tab) and atomic write.
- `src/fixer/fix.ts`: orchestrator that filters findings, applies patches, writes, rescans, and diffs.
...
- Dogfooded on a copy of the hexmetrics-real-world fixture: `"@esbuild/linux-x64": "latest"` rewritten to `">=0.25.12"`, post-fix rescan clean, exit 0.
...
- **OA006-COUPLED-PLATFORM-BINARY** severity is now tiered:
  - `high` when the target matches a platform-binary pattern (e.g. `@esbuild/<platform>`, `@next/swc-<platform>`, `@img/sharp-<platform>`, `lightningcss-<platform>`). The binary-coupling failure mode is severe.
  - `medium` for non-platform targets (e.g. `postcss`, `react`) when the override is currently effective. Pattern is risky but not actively broken.
...
- Scanner-level composite escalation step (OA006 + OA008 to high).
...
Adds three new detectors discovered via dogfooding v0.1.0 against `/home/aaron/Projects/hexmetrics`. The original `"@esbuild/linux-x64": "latest"` override revealed a class of override-hygiene bugs that no static analyser was catching, and these three rules close that gap.
...
- **`OA008-VULNERABLE-TWIN`** (critical): override declared a floor but a copy of the target package below that floor is *still on disk*. The override didn't actually apply, so the security pin is non-functional and the user is silently vulnerable. Catches the post-install gap that static analysis can't see. ([#7](https://github.com/Hexaxia-Labs/override-audit-cli/issues/7))
