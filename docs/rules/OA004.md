...
The fuller "walk the dep graph and check no parent depends on `<pin`" check is **deferred** (tracked as [#3](https://github.com/Hexaxia-Labs/override-audit-cli/issues/3)). v0.1.0+ uses a coarser heuristic:
...
```bash
override-audit --fix --rule OA004    # available since v0.2.0
...
For **cross-major** findings, the action stays `suggest` and no automated patch is produced — the explanation in the finding tells you what to verify before removing manually. `--fix` will skip these and surface them under `skippedFindings`.
