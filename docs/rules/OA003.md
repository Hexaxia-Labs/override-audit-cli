...
```bash
override-audit --fix --rule OA003    # available since v0.2.0
override-audit --fix --dry-run       # preview first
```

The fixer applies the RFC 6902 `move` patch and rewrites `package.json` in place (atomically — preserves your indent and trailing-newline style). Re-run `npm install` / `pnpm install` afterwards to actually apply the override at install time.
