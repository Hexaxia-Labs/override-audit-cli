...
The action is `suggest` (not `replace`) because the right fix is structural, not a single-key swap. The finding explanation provides the recommended parent override.
