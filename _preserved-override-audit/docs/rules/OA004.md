...
`parseInt("latest")` returns `NaN`. `NaN > X` is `false`. So overrides pinned to `"latest"` silently never got cleaned up, even when the resolved version had moved well past them.
