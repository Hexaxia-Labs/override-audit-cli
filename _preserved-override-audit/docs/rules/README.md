...
- **Human renderer** (`override-audit`) for severity-grouped plain text suitable for terminals and CI logs.
- **JSON renderer** (`override-audit --json`) for full structured output matching the v1 schema (locked via `tests/__snapshots__/output-snapshot.test.ts.snap`). Designed to be consumed by HexOps' `OverrideAuditSource` wrapper in v1.0.0.
